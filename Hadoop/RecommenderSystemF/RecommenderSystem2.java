...
public class RecommenderSystem2{
	public static class RecMap extends Mapper<Object, Text, Text, Text>{
		...
		public void map(Object key, Text value, Context context) 
			...
			for(int i = 0; i< st.length;i++){
				//we suppose input is in form: B,B1,B2,B4
				item.set(st[i]);
				if(i==st.length-1) boughtWith.set(line.replace(","+st[i],""));
				else boughtWith.set(line.replace(st[i]+",",""));
	...
	public static void main(String[] args) throws Exception{
		...
		job.setJarByClass(RecommenderSystem2.class);
